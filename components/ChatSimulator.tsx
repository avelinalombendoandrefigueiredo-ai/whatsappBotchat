...
export const ChatSimulator: React.FC<ChatSimulatorProps> = ({ menu, settings, onActivity, isConnected }) => {
  ...
  const handleSendMessage = async () => {
    ...
    if (cmd === 'voltar') {
        setTimeout(() => {
            if (!activeMenuId) {
                sendBotMessage("Você já está no menu principal.");
                setTimeout(() => sendAutoGeneratedMenu(menu), 800);
            } else {
    ...
    // Tenta encontrar correspondência exata do gatilho (ex: "1", "1.1")
    const matchedOption = currentOptions.find(opt => opt.trigger.trim().toLowerCase() === userText.toLowerCase());

    if (matchedOption) {
        await new Promise(r => setTimeout(r, 1000));

        // LÓGICA DE AUTOMATIZAÇÃO (PAI VS FILHO)
        const isParent = matchedOption.children.length > 0;

        if (isParent) {
            // Se é Pai (tem submenus): Envia texto de introdução + Lista automática
            setActiveMenuId(matchedOption.id);
            
            // Envia o texto introdutório configurado no pai
            sendBotMessage(matchedOption.content); // "Aqui estão nossos produtos:"
            ...
            // Se é Filho (Item Final): Envia a Mídia/Conteúdo Final
            ...
            // Se for TEXTO, o conteúdo vai no balão. Se for MÍDIA, o conteúdo é a URL e o texto é a legenda.
            ...
            // Opcional: Oferecer botão de voltar após conteúdo final
            setTimeout(() => {
                 sendBotMessage("_Digite 'Voltar' para retornar ao menu anterior ou 'Menu' para o início._");
            }, 1500);
        }
        ...
        try {
            ...
            // Mapear opções atuais para a IA saber o que sugerir
            ...
            console.error(e);
            sendBotMessage("Desculpe, não entendi. Por favor, digite o número da opção desejada.");
        }
    }
  };
  ...
  // Função que gera automaticamente a lista textual dos menus
  const sendAutoGeneratedMenu = (items: MenuItem[]) => {
      ...
      setTimeout(() => {
        ...
        // Cria lista formatada: 1. Opção X \n 2. Opção Y
        const optionsList = items.map(i => `*${i.trigger}* - ${i.title}`).join('\n');
        const footer = "\n\n_(Digite o número da opção desejada)_";
      ...
      return (
          <div className="flex items-center justify-center h-full bg-gray-100 p-6 animate-in fade-in">
              <div className="bg-white p-8 rounded-xl shadow-lg text-center max-w-md border border-gray-200">
                  ...
                  <p className="text-gray-500 mb-6">Para garantir que os dados sejam reais, você precisa conectar o WhatsApp via QR Code primeiro.</p>
            ...
            <div className="bg-[#f0f2f5] px-2 py-2 flex items-end gap-2 z-20 select-none pb-4 md:pb-2">
                <div className="bg-white rounded-2xl flex-1 flex items-end p-2 shadow-sm border border-gray-200 focus-within:border-green-500 transition-colors">
                    ...
                    <textarea
                        ...
                        placeholder={isBlockedUser ? "Modo manual (Bot pausado)" : "Digite uma opção ou mensagem..."}
                        rows={1}
